# eXpOS NITC OS Lab

This repo contains the files and source code for my SEM4 OS Lab, the eXpOS (eXperimental Operating System) NITC.

## Overview
### [Roadmap](https://exposnitc.github.io/Roadmap.html)
### Preperatory stages
- [x] Stage 1 - [Setting up the system]
- [x] Stage 2 - [Understanding the file system]
- [x] Stage 3 - [Bootstrap loader]
- [x] Stage 4 - [Learning the SPL language]
- [x] Stage 5 - [XSM debugging]
- [x] Stage 6 - [Running a user program]
- [x] Stage 7 - [ABI and XEXE format]
- [x] Stage 8 - [Handling timer interrupt]
- [x] Stage 9 - [Handling kernel stack]
- [x] Stage 10 - [Console output]
- [x] Stage 11 - [Introduction to ExpL]
- [x] Stage 12 - [Introduction to multiprogramming]

### Intermediate stages
- [x] Stage 13 - [Boot module]
- [x] Stage 14 - [Round robin scheduler]
- [x] Stage 15 - [Resource manager module]
- [ ] Stage 16 - [Console input]
- [ ] Stage 17 - [Program loader]
- [ ] Stage 18 - [Disk interrup handler]
- [ ] Stage 19 - [Exception handler]

### Final stages
- [ ] Stage 20 - [Process creation and termination]
- [ ] Stage 21 - [Process synchronization]
- [ ] Stage 22 - [Semaphores]
- [ ] Stage 23 - [File creation and deletion]
- [ ] Stage 24 - [File read]
- [ ] Stage 25 - [File write]
- [ ] Stage 26 - [User management]
- [ ] Stage 27 - [Pager module]
- [ ] Stage 28 - [Multi-core extension]

## Resources
* Documentaion - [eXpOSNITC](https://exposnitc.github.io/index.html)
